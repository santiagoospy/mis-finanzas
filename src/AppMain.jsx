...
function Pie({data}) {
  ...
  return React.createElement("div",null,
    React.createElement("svg",{viewBox:"0 0 200 200",width:"100%",style:{maxWidth:200,display:"block",margin:"0 auto"}},
      sl.map((s,i)=>React.createElement("path",{key:i,d:s.path,fill:s.c,stroke:"#fff",strokeWidth:"1.5"})),
      React.createElement("text",{x:"100",y:"94",textAnchor:"middle",fontSize:"11",fill:TXS},"Total"),
      React.createElement("text",{x:"100",y:"110",textAnchor:"middle",fontSize:"10",fontWeight:"500",fill:TX},fmt(tot))
    ),
    React.createElement("div",{style:{display:"flex",flexWrap:"wrap",gap:"5px 10px",marginTop:10,justifyContent:"center"}},
      sl.map((s,i)=>React.createElement("div",{key:i,style:{display:"flex",alignItems:"center",gap:4}},
        React.createElement("div",{style:{width:9,height:9,borderRadius:2,background:s.c}}),
        React.createElement("span",{style:{fontSize:11,color:TXS}},s.label+" ",React.createElement("b",{style:{color:TX,fontWeight:500}},Math.round(s.p*100)+"%"))
      ))
    )
  );
}
...
const Sheet = ({onClose,title,children}) =>
  React.createElement("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"flex-end",zIndex:100},onClick:e=>e.target===e.currentTarget&&onClose()},
    React.createElement("div",{style:{background:BG,borderRadius:"20px 20px 0 0",width:"100%",padding:"20px 16px 40px",maxHeight:"88vh",overflowY:"auto",boxSizing:"border-box"}},
      React.createElement("p",{style:{fontSize:17,fontWeight:500,marginBottom:14,color:TX}},title),
      children
    )
  );
...
export default function App() {
  ...
  const applyData = d => { if(!d) return; if(d.accs) setAccs(d.accs); if(d.txs) setTxs(d.txs); if(d.recs) setRecs(d.recs); if(d.iCats) setICats(d.iCats); if(d.eCats) setECats(d.eCats); };

  useEffect(()=>{
    try { const r=localStorage.getItem(KEY); if(r) applyData(JSON.parse(r)); } catch{}
    initGoogle(CLIENT_ID)
      .then(()=>setStatus("Conectar con Google Drive"))
      .catch(()=>setStatus("Error al cargar Google"));
  },[]);
  ...
  const pieData = Object.entries(vis.filter(t=>t.type==="gasto").reduce((m,t)=>Object.assign({},m,{[t.category]:(m[t.category]||0)+t.amount}),{})).sort((a,b)=>b[1]-a[1]).map(function(e){return{label:e[0],value:e[1]};});
  ...
  const btn = (label,onClick,color,outline) => React.createElement("button",{onClick,style:{padding:"12px",background:outline?"transparent":color||G,color:outline?color||G:"#fff",border:"1.5px solid "+(color||G),borderRadius:10,fontSize:14,fontWeight:500,cursor:"pointer",width:"100%",marginBottom:8}},label);
  const inp = (props) => React.createElement("input",Object.assign({},props,{style:{width:"100%",boxSizing:"border-box",marginBottom:8,background:BGS,color:TX,border:"1px solid "+BD,borderRadius:8,padding:"9px 10px",fontSize:15}}));
  const sel = (props,children) => React.createElement("select",Object.assign({},props,{style:{width:"100%",boxSizing:"border-box",marginBottom:8,background:BGS,color:TX,border:"1px solid "+BD,borderRadius:8,padding:"9px 10px",fontSize:15}}),children);
  const lbl = text => React.createElement("label",{style:{fontSize:13,color:TXS,marginBottom:4,display:"block",marginTop:6}},text);
  const toggle = (val,onChange) => React.createElement("div",{style:{display:"flex",background:BGS,borderRadius:10,padding:3,marginBottom:12}},
    [["gasto","Gasto",R],["ingreso","Ingreso",G]].map(([v,l,c])=>React.createElement("button",{key:v,onClick:()=>onChange(v),style:{flex:1,padding:"8px",border:"none",borderRadius:8,fontWeight:500,fontSize:14,cursor:"pointer",background:val===v?c:"transparent",color:val===v?"#fff":TXS}},l))
  );

  return React.createElement("div",{style:{fontFamily:"system-ui,-apple-system,sans-serif",maxWidth:480,margin:"0 auto",paddingBottom:80,background:BG,minHeight:"100vh"}},
    toast&&React.createElement("div",{style:{position:"fixed",bottom:90,left:"50%",transform:"translateX(-50%)",background:"#1a1a1a",color:"#fff",fontSize:13,padding:"10px 20px",borderRadius:20,zIndex:200,whiteSpace:"nowrap"}},toast),

    React.createElement("div",{style:{borderBottom:"1px solid "+BD,display:"flex",background:BG}},
      [["home","Finanzas"],["rec","Por cobrar"],["bak","Drive"]].map(([v,l])=>
        React.createElement("button",{key:v,onClick:()=>{setView(v);if(v==="home")setSelAcc(null);},style:{flex:1,padding:"12px 0",border:"none",background:"none",fontWeight:view===v?500:400,fontSize:14,color:view===v?G:TXS,borderBottom:view===v?"2px solid "+G:"2px solid transparent",cursor:"pointer"}},
          l,v==="rec"&&pRecs.length>0&&React.createElement("span",{style:{background:R,color:"#fff",borderRadius:10,fontSize:11,padding:"1px 5px",marginLeft:3}},pRecs.length)
        )
      )
    ),
    React.createElement("div",{style:{textAlign:"center",fontSize:11,color:TXS,padding:"3px 0",background:BGS}},status),

    view==="home"&&React.createElement("div",null,
      selAcc&&React.createElement("div",{style:{padding:"10px 16px 0"}},React.createElement("button",{style:{background:"none",border:"none",color:G,fontSize:14,cursor:"pointer",padding:0},onClick:()=>{setSelAcc(null);setTab("todos");}},"\u2190 Volver")),
      React.createElement("div",{style:{margin:"14px 16px",borderRadius:16,padding:"18px 20px",background:"linear-gradient(135deg,"+G+" 0%,"+GD+" 100%)"}},
        React.createElement("p",{style:{fontSize:12,color:"rgba(255,255,255,0.8)",marginBottom:3}},(selAcc?"Saldo actual":"Saldo total")+" - "+mLabel(selMonth)),
        React.createElement("p",{style:{fontSize:30,fontWeight:500,color:"#fff",margin:0}},fmt(selAcc?selAcc.balance:totBal)),
        React.createElement("div",{style:{display:"flex",gap:8,marginTop:12}},
          [["+"+fmt(totInc),"Ingresos"],["-"+fmt(totExp),"Gastos"],[fmt(totPend),"Por cobrar"]].map(([v,l])=>
            React.createElement("div",{key:l,style:{flex:1,background:"rgba(255,255,255,0.15)",borderRadius:10,padding:"9px 10px"}},
              React.createElement("p",{style:{fontSize:11,color:"rgba(255,255,255,0.75)",margin:0}},l),
              React.createElement("p",{style:{fontSize:13,fontWeight:500,color:"#fff",margin:"2px 0 0"}},v)
            )
          )
        )
      ),
      !selAcc&&React.createElement("div",null,
        React.createElement("p",{style:{fontSize:12,fontWeight:500,color:TXS,padding:"12px 16px 6px",textTransform:"uppercase",letterSpacing:0.5,margin:0}},"Cuentas"),
        accs.map(a=>React.createElement("div",{key:a.id,style:{margin:"0 16px 8px",background:BG,border:"1px solid "+BD,borderRadius:12,padding:"12px 14px",display:"flex",alignItems:"center",gap:12}},
          React.createElement("div",{style:{width:38,height:38,borderRadius:10,background:a.color+"33",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,cursor:"pointer"},onClick:()=>setSelAcc(a)},ICONS[a.type]||"💰"),
          React.createElement("div",{style:{flex:1,cursor:"pointer"},onClick:()=>setSelAcc(a)},
            React.createElement("p",{style:{fontSize:15,fontWeight:500,color:TX,margin:0}},a.name),
            React.createElement("p",{style:{fontSize:12,color:TXS,margin:0}},a.type==="bank"?"Cuenta bancaria":a.type==="credit"?"Tarjeta de credito":"Billetera")
          ),
          React.createElement("div",{style:{textAlign:"right"}},
            React.createElement("p",{style:{fontSize:15,fontWeight:500,color:a.balance>=0?TX:R,margin:0}},fmt(a.balance)),
            React.createElement("p",{style:{fontSize:11,color:TXS,margin:0}},"inicial: "+fmt(a.initialBalance||0))
          ),
          React.createElement("button",{style:{background:"none",border:"none",color:TXS,cursor:"pointer",fontSize:14,padding:"4px 6px"},onClick:()=>{setEditAccForm(Object.assign({},a,{_orig:a.initialBalance||0}));setModal("editAcc");}},"\u270F\uFE0F")
        )),
        accs.length<5&&React.createElement("button",{style:{margin:"4px 16px 0",background:"none",border:"1px dashed rgba(0,0,0,0.18)",borderRadius:12,padding:"10px",width:"calc(100% - 32px)",color:TXS,fontSize:14,cursor:"pointer"},onClick:()=>setModal("addAcc")},"+ Agregar cuenta")
      ),
      React.createElement("div",{style:{display:"flex",gap:6,padding:"10px 16px 2px",overflowX:"auto"}},
        months.map(m=>React.createElement("button",{key:m,style:{padding:"5px 14px",borderRadius:20,border:"none",cursor:"pointer",whiteSpace:"nowrap",fontSize:13,fontWeight:selMonth===m?500:400,background:selMonth===m?G:BGS,color:selMonth===m?"#fff":TXS,flexShrink:0},onClick:()=>setSelMonth(m)},mLabel(m)))
      ),
      React.createElement("div",{style:{fontSize:12,fontWeight:500,color:TXS,padding:"12px 16px 6px",textTransform:"uppercase",letterSpacing:0.5,display:"flex",justifyContent:"space-between",alignItems:"center"}},
        React.createElement("span",null,"Movimientos"),
        React.createElement("div",{style:{display:"flex",gap:8}},
          React.createElement("button",{style:{fontSize:12,color:G,background:"none",border:"none",cursor:"pointer",fontWeight:500},onClick:()=>{setCatType("ingreso");setModal("cats");}},"+ Ingresos"),
          React.createElement("button",{style:{fontSize:12,color:G,background:"none",border:"none",cursor:"pointer",fontWeight:500},onClick:()=>{setCatType("gasto");setModal("cats");}},"+ Gastos")
        )
      ),
      React.createElement("div",{style:{display:"flex",borderBottom:"1px solid "+BD,padding:"0 16px",background:BG}},
        ["todos","ingreso","gasto"].map(t=>React.createElement("button",{key:t,style:{padding:"9px 12px",fontSize:13,border:"none",background:"none",cursor:"pointer",color:tab===t?G:TXS,fontWeight:tab===t?500:400,borderBottom:tab===t?"2px solid "+G:"2px solid transparent"},onClick:()=>setTab(t)},t==="todos"?"Todos":t==="ingreso"?"Ingresos":"Gastos"))
      ),
      filt.length===0?React.createElement("p",{style:{textAlign:"center",color:TXS,padding:"28px 16px",fontSize:14}},"Sin movimientos en "+mLabel(selMonth))
      :filt.map(tx=>{const acc=accs.find(a=>a.id===tx.accountId);return React.createElement("div",{key:tx.id,style:{display:"flex",alignItems:"center",gap:10,padding:"11px 16px",borderBottom:"1px solid "+BD,background:BG}},
        React.createElement("div",{style:{width:36,height:36,borderRadius:9,background:tx.type==="ingreso"?GL:"#FAECE7",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}},EMOJI[tx.category]||"📌"),
        React.createElement("div",{style:{flex:1,minWidth:0}},
          React.createElement("p",{style:{fontSize:14,fontWeight:500,color:TX,margin:0}},tx.category),
          React.createElement("p",{style:{fontSize:12,color:TXS,margin:0}},[tx.description,acc&&acc.name,tx.date].filter(Boolean).join(" - "))
        ),
        React.createElement("p",{style:{marginLeft:"auto",fontSize:14,fontWeight:500,color:tx.type==="ingreso"?G:R,whiteSpace:"nowrap"}},(tx.type==="ingreso"?"+":"-")+fmt(tx.amount)),
        React.createElement("button",{style:{background:"none",border:"none",color:TXS,cursor:"pointer",fontSize:14,padding:"4px",marginLeft:4},onClick:()=>delTx(tx.id)},"\u2715")
      );}),
      pieData.length>0&&React.createElement("div",{style:{margin:"16px 16px 8px",background:BG,border:"1px solid "+BD,borderRadius:14,padding:"16px"}},
        React.createElement("p",{style:{fontSize:14,fontWeight:500,color:TX,marginBottom:14,textAlign:"center"}},"Gastos - "+mLabel(selMonth)),
        React.createElement(Pie,{data:pieData})
      )
    ),

    view==="rec"&&React.createElement("div",null,
      React.createElement("div",{style:{margin:"14px 16px",borderRadius:16,padding:"18px 20px",background:"linear-gradient(135deg,"+P+" 0%,"+PD+" 100%)"}},
        React.createElement("p",{style:{fontSize:12,color:"rgba(255,255,255,0.8)",marginBottom:3}},"Total por cobrar"),
        React.createElement("p",{style:{fontSize:30,fontWeight:500,color:"#fff",margin:0}},fmt(totPend)),
        React.createElement("div",{style:{display:"flex",gap:8,marginTop:12}},
      ...
      pRecs.length>0&&React.createElement("p",{style:{fontSize:12,fontWeight:500,color:TXS,padding:"12px 16px 6px",textTransform:"uppercase",letterSpacing:0.5,margin:0}},"Pendientes"),
      pRecs.map(r=>React.createElement("div",{key:r.id,style:{margin:"0 16px 8px",background:BG,border:"1px solid "+BD,borderRadius:12,padding:"12px 14px"}},
        React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}},
          React.createElement("div",null,
            React.createElement("p",{style:{fontSize:15,fontWeight:500,color:TX,margin:0}},r.client),
            r.concept&&React.createElement("p",{style:{fontSize:13,color:TXS,margin:"2px 0 0"}},r.concept),
            r.dueDate&&React.createElement("p",{style:{fontSize:12,color:TXS,margin:"2px 0 0"}},"Vence: "+r.dueDate)
          ),
          React.createElement("div",{style:{textAlign:"right"}},
            React.createElement("p",{style:{fontSize:16,fontWeight:500,color:P,margin:0}},fmt(r.amount)),
            React.createElement("span",{style:{fontSize:11,fontWeight:500,padding:"2px 8px",borderRadius:6,background:AML,color:AM}},r.status)
          )
        ),
        r.notes&&React.createElement("p",{style:{fontSize:12,color:TXS,marginTop:6,marginBottom:0}},r.notes),
        React.createElement("div",{style:{display:"flex",gap:6,marginTop:8}},
          React.createElement("button",{style:{flex:1,padding:"7px",border:"1px solid "+G,borderRadius:8,background:"none",color:G,fontSize:12,fontWeight:500,cursor:"pointer"},onClick:()=>markRec(r.id,"cobrado")},"\u2713 Cobrado"),
          React.createElement("button",{style:{flex:1,padding:"7px",border:"1px solid "+R,borderRadius:8,background:"none",color:R,fontSize:12,fontWeight:500,cursor:"pointer"},onClick:()=>markRec(r.id,"cancelado")},"\u2717 Cancelado"),
          React.createElement("button",{style:{flex:1,padding:"7px",border:"1px solid #888",borderRadius:8,background:"none",color:"#888",fontSize:12,fontWeight:500,cursor:"pointer"},onClick:()=>delRec(r.id)},"Eliminar")
        )
      )),
      oRecs.length>0&&React.createElement("p",{style:{fontSize:12,fontWeight:500,color:TXS,padding:"12px 16px 6px",textTransform:"uppercase",letterSpacing:0.5,margin:0}},"Historial"),
      oRecs.map(r=>React.createElement("div",{key:r.id,style:{margin:"0 16px 8px",background:BG,border:"1px solid "+BD,borderRadius:12,padding:"12px 14px",opacity:0.7}},
        React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center"}},
          React.createElement("div",null,
            React.createElement("p",{style:{fontSize:14,fontWeight:500,color:TX,margin:0}},r.client),
            r.concept&&React.createElement("p",{style:{fontSize:12,color:TXS,margin:"2px 0 0"}},r.concept)
          ),
          React.createElement("div",{style:{textAlign:"right"}},
            React.createElement("p",{style:{fontSize:14,fontWeight:500,color:TXS,margin:0}},fmt(r.amount)),
            React.createElement("span",{style:{fontSize:11,fontWeight:500,padding:"2px 8px",borderRadius:6,background:r.status==="cobrado"?GL:"#FCEBEB",color:r.status==="cobrado"?GD:"#A32D2D"}},r.status)
          )
        ),
        React.createElement("div",{style:{display:"flex",gap:6,marginTop:8}},
          React.createElement("button",{style:{flex:1,padding:"7px",border:"1px solid "+P,borderRadius:8,background:"none",color:P,fontSize:12,fontWeight:500,cursor:"pointer"},onClick:()=>markRec(r.id,"pendiente")},"Reabrir"),
          React.createElement("button",{style:{flex:1,padding:"7px",border:"1px solid #888",borderRadius:8,background:"none",color:"#888",fontSize:12,fontWeight:500,cursor:"pointer"},onClick:()=>delRec(r.id)},"Eliminar")
        )
      )),
      recs.length===0&&React.createElement("p",{style:{textAlign:"center",color:TXS,padding:"28px 16px",fontSize:14}},"Sin trabajos por cobrar")
    ),

    view==="bak"&&React.createElement("div",null,
      React.createElement("div",{style:{padding:"20px 16px 8px"}},
        React.createElement("p",{style:{fontSize:20,fontWeight:500,color:TX,margin:"0 0 4px"}},"Google Drive"),
        React.createElement("p",{style:{fontSize:14,color:TXS,margin:0}},"Sincroniza tus datos automaticamente.")
      ),
      React.createElement("div",{style:{margin:"12px 16px",background:signedIn?GL:AML,borderRadius:12,padding:"14px"}},
        React.createElement("p",{style:{fontSize:14,fontWeight:500,color:signedIn?GD:AM,margin:"0 0 2px"}},signedIn?"Conectado a Google Drive":"Sin conexion a Drive"),
        React.createElement("p",{style:{fontSize:13,color:signedIn?GD:AM,margin:0}},signedIn?"Los datos se guardan automaticamente en tu Drive.":"Conectate para sincronizar entre dispositivos.")
      ),
      !signedIn&&React.createElement("div",{style:{margin:"12px 16px"}},
        React.createElement("button",{onClick:handleSignIn,style:{width:"100%",padding:"14px",background:G,color:"#fff",border:"none",borderRadius:12,fontSize:16,fontWeight:500,cursor:"pointer"}},"Conectar con Google Drive")
      ),
    ...
    view!=="bak"&&React.createElement("button",{style:{position:"fixed",bottom:24,right:24,width:54,height:54,borderRadius:27,background:G,border:"none",color:"#fff",fontSize:26,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",zIndex:50},onClick:()=>view==="rec"?setModal("addRec"):openTxForm(selAcc&&selAcc.id)},"+"),

    modal==="tx"&&React.createElement(Sheet,{onClose:()=>setModal(null),title:"Nueva transaccion"},
      toggle(form.type,v=>setForm(Object.assign({},form,{type:v,category:(v==="ingreso"?iCats:eCats)[0]||""}))),
      lbl("Cuenta"),sel({value:form.accountId||"",onChange:e=>setForm(Object.assign({},form,{accountId:e.target.value}))},accs.map(a=>React.createElement("option",{key:a.id,value:a.id},a.name))),
      lbl("Categoria"),sel({value:form.category,onChange:e=>setForm(Object.assign({},form,{category:e.target.value}))},(form.type==="ingreso"?iCats:eCats).map(c=>React.createElement("option",{key:c},c))),
      lbl("Monto (Gs)"),inp({type:"number",placeholder:"0",value:form.amount,onChange:e=>setForm(Object.assign({},form,{amount:e.target.value}))}),
      lbl("Descripcion (opcional)"),inp({type:"text",placeholder:"Ej: Superseis",value:form.description,onChange:e=>setForm(Object.assign({},form,{description:e.target.value}))}),
      lbl("Fecha"),inp({type:"date",value:form.date,onChange:e=>setForm(Object.assign({},form,{date:e.target.value}))}),
      btn("Guardar",addTx)
    ),

    modal==="addAcc"&&React.createElement(Sheet,{onClose:()=>setModal(null),title:"Nueva cuenta"},
      ...
      lbl("Tipo"),sel({value:accForm.type,onChange:e=>setAccForm(Object.assign({},accForm,{type:e.target.value}))},
        React.createElement("option",{value:"bank"},"Cuenta bancaria"),
        React.createElement("option",{value:"credit"},"Tarjeta de credito"),
        React.createElement("option",{value:"wallet"},"Billetera")
      ),
    ...
    modal==="editAcc"&&editAccForm&&React.createElement(Sheet,{onClose:()=>setModal(null),title:"Editar cuenta"},
    ...
    modal==="addRec"&&React.createElement(Sheet,{onClose:()=>setModal(null),title:"Nuevo trabajo por cobrar"},
      ...
      lbl("Concepto"),inp({type:"text",placeholder:"Ej: Diseno, Consultoria",value:recForm.concept,onChange:e=>setRecForm(Object.assign({},recForm,{concept:e.target.value}))}),
      ...
      lbl("Notas (opcional)"),inp({type:"text",placeholder:"Ej: Acordado por WhatsApp",value:recForm.notes,onChange:e=>setRecForm(Object.assign({},recForm,{notes:e.target.value}))}),
      btn("Guardar",addRec)
    ),

    modal==="cats"&&React.createElement(Sheet,{onClose:()=>setModal(null),title:"Categorias"},
      toggle(catType,v=>{setCatType(v);setEditCat(null);setNewCat("");}),
      curCats.map(cat=>React.createElement("div",{key:cat,style:{display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderBottom:"1px solid "+BD}},
        editCat===cat
          ?React.createElement("input",{autoFocus:true,style:{flex:1,padding:"5px 8px",border:"1px solid rgba(0,0,0,0.18)",borderRadius:6,fontSize:14,background:BGS,color:TX},defaultValue:cat,onBlur:e=>renameCat(cat,e.target.value),onKeyDown:e=>{if(e.key==="Enter")renameCat(cat,e.target.value);if(e.key==="Escape")setEditCat(null);}})
          :React.createElement("span",{style:{flex:1,fontSize:14,color:TX}},(EMOJI[cat]||"📌")+" "+cat),
        React.createElement("button",{style:{background:"none",border:"none",color:TXS,cursor:"pointer",fontSize:14,padding:"4px 6px"},onClick:()=>setEditCat(cat)},"\u270F\uFE0F"),
        React.createElement("button",{style:{background:"none",border:"none",color:R,cursor:"pointer",fontSize:14,padding:"4px 6px"},onClick:()=>delCat(cat)},"\u2715")
      )),
      React.createElement("div",{style:{display:"flex",gap:8,marginTop:14}},
        React.createElement("input",{style:{flex:1,padding:"8px 10px",border:"1px solid rgba(0,0,0,0.18)",borderRadius:8,fontSize:14,background:BGS,color:TX},placeholder:"Nueva categoria...",value:newCat,onChange:e=>setNewCat(e.target.value),onKeyDown:e=>e.key==="Enter"&&addCat()}),
        React.createElement("button",{style:{padding:"8px 16px",background:G,color:"#fff",border:"none",borderRadius:8,fontWeight:500,cursor:"pointer"},onClick:addCat},"Agregar")
      )
    )
  );
}
