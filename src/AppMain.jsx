...
const Sheet=({onClose,title,children})=>ce("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"flex-end",zIndex:100},onClick:e=>e.target===e.currentTarget&&onClose()},
  ce("div",{style:{background:BG,borderRadius:"20px 20px 0 0",width:"100%",padding:"20px 16px 40px",maxHeight:"88vh",overflowY:"auto",boxSizing:"border-box"}},
    ce("p",{style:{fontSize:17,fontWeight:500,marginBottom:14,color:TX}},title),children));
...
export default function App() {
  ...
  const applyData=d=>{
    if(!d) return;
    const a=d.accs||d.accounts,t=d.txs||d.transactions,r=d.recs||d.receivables;
    const ic=d.iCats||d.incomeCats,ec=d.eCats||d.expenseCats;
  ...
  useEffect(()=>{
    ...
    initGoogle(CLIENT_ID).then(()=>setStatus("Listo")).catch(()=>setStatus("Sin Google"));
  },[]);
  ...
  const importData=()=>{
    if(!importVal.trim()){alert("Pega el JSON primero");return;}
    try{
      const s=importVal.indexOf("{"),e=importVal.lastIndexOf("}");
      const d=JSON.parse(importVal.slice(s,e+1));
      applyData(d);
      save(d.accs||d.accounts,d.txs||d.transactions,d.recs||d.receivables,d.iCats||d.incomeCats,d.eCats||d.expenseCats);
  ...
  const btn=(label,onClick,col,outline)=>ce("button",{onClick,style:{padding:"12px",background:outline?"transparent":col||G,color:outline?col||G:"#fff",border:"1px solid "+(col||G),borderRadius:10,fontSize:14,fontWeight:500,cursor:"pointer",width:"100%",marginBottom:8}},label);
  ...
  const card=(children,extra)=>ce("div",{style:Object.assign({margin:"12px 16px",background:BGS,borderRadius:12,padding:"16px",border:"1px solid "+BD},extra)},children);
  ...
  return ce("div",{style:{fontFamily:"system-ui,-apple-system,sans-serif",maxWidth:480,margin:"0 auto",paddingBottom:80,background:BG,minHeight:"100vh"}},
    ...
    view==="bak"&&ce("div",null,
      ...
      signedIn&&ce("div",{style:{margin:"12px 16px",background:BGS,borderRadius:12,padding:"12px 16px",border:"1px solid "+BD}},
      ...
      card(ce("div",null,
        ...
        ce("p",{style:{fontSize:13,color:TXS,marginBottom:8}},accs.length+" cuentas - "+txs.length+" movimientos"),
        btn("Mostrar mis datos",()=>setExportJson(JSON.stringify({accs,txs,recs,iCats,eCats},null,2)),G,true),
        exportJson&&ce("textarea",{readOnly:true,value:exportJson,onFocus:e=>e.target.select(),style:{width:"100%",boxSizing:"border-box",height:80,resize:"none",fontSize:11,fontFamily:"monospace",background:"#fff",color:TX,border:"1px solid "+BD,borderRadius:8,padding:"8px",marginTop:4}}))),
      card(ce("div",null,
        ce("p",{style:{fontSize:14,fontWeight:500,color:TX,marginBottom:4}},"Importar datos"),
        ce("p",{style:{fontSize:13,color:TXS,marginBottom:8}},"Pega tu JSON para restaurar tus datos."),
        ce("textarea",{value:importVal,onChange:e=>setImportVal(e.target.value),style:{width:"100%",boxSizing:"border-box",marginBottom:8,height:100,resize:"none",fontSize:12,fontFamily:"monospace",background:"#fff",color:TX,border:"1px solid "+BD,borderRadius:8,padding:"9px 10px"},placeholder:"Pega el JSON aqui..."}),
        btn("Importar",importData)))),
